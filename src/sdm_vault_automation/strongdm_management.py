...
def get_accountattachment(role_id, account_id):
    ...
    get first object by name
    '''
    aas = [i for i in get_client().account_attachments.list('account_id:{},role_id:{}'.format(account_id,role_id))]
    for i in aas:
...
def delete_resource_by_hostname(name):
    ...
    '''
    nodes = [i for i in get_client().resources.list('hostname:{}'.format(name))]
    return [get_client().resources.delete(i.id) for i in nodes]
...
def delete_nodes_by_name(name):
    ...
    delete relay nodes by name
    '''
    nodes = [i.name == name for i in get_client().nodes.list('name:{}'.format(name))]
...
def delete_nodes_by_hostname(hostname):
    ...
    nodes = [i for i in get_client().nodes.list('hostname:{}'.format(hostname))]
    return [get_client().nodes.delete(i).to_dict() for i in nodes]
...
def add_resource_access(rolename, resource_names=[]):
    ...
    for name in resource_names:
        nodes = nodes + [i for i in get_client().resources.list('name:{}'.format(name))]
...
def create_account_attachment_by_names(rolename, user_email):
    ...
    users = [i for i in get_client().accounts.list('email:{}'.format(user_email))]
    if len(users) == 0:
...
def add_user_access(rolename, user_emails=[]):
    ...
    for email in user_emails:
        users = users + [i for i in get_client().accounts.list('email:{}'.format(email))]
    
    results = []
    aas = []
    for user in users:
        i = get_client().account_attachments.list('account_id:{},role_id:{}'.format(user.id, role.id))
...
def remove_user_access(rolename, user_emails=[]):
    ...
    aas = []
    for user in users:
        aas = aas + [i for i in get_client().account_attachments.list('account_id:{},role_id:{}'.format(user.id, role.id))]
...
def assign_new_secret_store(old_secret_store, new_secret_store, server_names=[]):
    ...
    if len(server_names) == 0:
        # assign all servers
        servers = [i for i in get_client().resources.list('secret_store_id:{}'.format(old_ss.secret_store_id))]
